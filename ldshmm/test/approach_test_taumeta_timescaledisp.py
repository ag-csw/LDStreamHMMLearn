...
class Approach_Test(TestCase):
    ...
    def test_taumeta_num_traj(self):
        ...
        timescale_disp_values = [2,4,8]

        for one, taumeta in enumerate(taumeta_values):
            for two, timescale_disp in enumerate(timescale_disp_values):

                self.mmf1_0 = MMFamily1(self.nstates, timescale_disp)
        ...
        plot_result_heatmap(avg_times_naive, avg_times_bayes, taumeta_values, timescale_disp_values,"timescaledisp", "Performance", "Heatmap Performance Taumeta Timescaledisp")
        plot_result_heatmap(avg_errs_naive, avg_errs_bayes, taumeta_values, timescale_disp_values, "timescaledisp","Error", "Heatmap Error Taumeta Timescaledisp")
